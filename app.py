...
def dashboard():
    ...
    return render_template('dashboard.html', accuracy=accuracy, global_importance_plot=global_importance_plot, pdp_plot=pdp_plot, errors=errors)
    ...
    if 'csv_file' in request.files and request.files['csv_file'].filename != '':
        ...
        if not errors:
            try:
                ...
                file_exists = os.path.exists(retraining_file_path)
                df_new_row.to_csv(retraining_file_path, mode='a', header=not file_exists, index=False)
